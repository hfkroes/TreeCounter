...
public class system extends ModuleBody {
    ...
    public static final ModuleMethod f266catch;
    ...
    static {
        ...
        f266catch = new ModuleMethod(system2, 9, Lit10, 12291);
    ...
    public static Process makeProcess(Object args, Object env) {
        Object arargs = vectors.isVector(args) ? convertVectorToStringArray(args) : C0619lists.isList(args) ? convertListToStringArray(args) : strings.isString(args) ? command$Mnparse.apply1(args) : args instanceof String[] ? args : misc.error$V("invalid arguments to make-process", new Object[0]);
        try {
            try {
    ...
    public static Object convertListToStringArray(Object lst) {
        try {
            String[] arr = new String[C0619lists.length((LList) lst)];
            int i = 0;
            Object p = lst;
            while (!C0619lists.isNull(p)) {
                try {
                    Pair pp = (Pair) p;
    ...
    public static Object tokenizeStringToStringArray(String string) {
        ...
        while (toks.hasMoreTokens()) {
            rlist = C0619lists.cons(toks.nextToken(), rlist);
        }
        try {
            int count = C0619lists.length((LList) rlist);
            String[] arr = new String[count];
            int i = count - 1;
            Object p = rlist;
            while (!C0619lists.isNull(p)) {
